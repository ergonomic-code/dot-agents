...
[`$advance-task`](src/skills/advance-task/SKILL.md) определяет по артефактам, Git-состоянию и проверенным результатам следующую стадию текущего инкремента, выполняет только её и останавливается.
Каждый следующий вызов заново проверяет состояние и при доказанных предпосылках выполняет одну следующую стадию; при конфликте или неполных доказательствах он останавливается с причиной.
