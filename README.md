...
flowchart TD
    ...
    Baseline --> TaskResolver[Резолвер задачи<br/>task-workdir/resolve_task.py]
    TaskResolver -. при наличии задачи .-> TaskModule[Модуль task-workdir<br/>task-workdir/context.md]
    TaskModule --> Role
...
В первом ходе baseline один раз передаёт исходный запрос детерминированному резолверу задачи.
...
Ветка, cwd, slug, произвольные числа и общая смысловая похожесть не активируют задачу.
...
| `src/task-workdir/` | Содержит резолвер задачи, условный контекст, task-workdir-скиллы и шаблоны стандартных артефактов задачи. |
