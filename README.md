...
В этом режиме разработка идёт по спецификации, но на первом этапе описываются только треобования и общие направление и ограничения решения, а затем они уточняются, дополняются и, при необходимости, изменяются по мере реализации.

#### Подготовка задачи

1. Подготовьте рабочую директорию через [`$prepare-task-workdir`](src/skills/prepare-task-workdir/SKILL.md).
...
3. Разбейте реализацию на проверяемые вертикальные инкременты в `todo.md`.
...
5. Спроектируйте и спланируйте реализацию кейса через [`$plan-test-case-fixing`](src/skills/plan-test-case-fixing/SKILL.md).
7. Сделайте тест зелёным минимальным изменением production-кода через [`$fix-red-case`](src/skills/fix-red-case/SKILL.md), добавив в промт план реализации.
8. Выполните ограниченный рефакторинг зелёного инкремента через [`$refactor-case`](src/skills/refactor-case/SKILL.md).
9. Обновите `todo.md` и повторите цикл для следующего инкремента.

Каждый инкремент изменяет спецификацию и код только в пределах выбранного поведения.

#### Пошаговая автоматизация

Выбор и выполнение следующего шага итерации инкремента можно автоматизировать с помощью [`$advance-task`](src/skills/advance-task/SKILL.md).
Он определяет по артефактам, Git-состоянию и проверенным результатам следующую стадию текущего инкремента, выполняет только её и останавливается.
...
| Скилл | Назначение |
| --- | --- |
| [`$advance-task`](src/skills/advance-task/SKILL.md) | Определяет и выполняет одну следующую доказуемую стадию TDD-инкремента. |
...
| [`$prepare-task-workdir`](src/skills/prepare-task-workdir/SKILL.md) | В диалоге подготавливает бриф задачи, необходимые кодовые якоря и выбранное направление решения. |
