...
Помимо этого вы можете взять фреймворк в качестве основы, а потом "доработать напильником" или полностью заменить отдельные его части: роутинг контекста, конвенции дизайна и кодирования, конвенции процесса работы, скиллы и артефакты.
...
### Работа над большими задачами в нескольких сессиях

Параллельно с общими соглашениями и скиллами, фреймворк поддерживает работу над большими задачами в нескольких сессиях.
В этом случае весь необходимый контекст собирается в одной директории и агент автоматически его использует, дополняет и обновляет по мере работы.

Фреймворк предполагает работу над большими задачами по легковесному варианту SDD:

1. на первом этапе описываются только основные треобования, а так же общие направление и ограничения решения.
2. затем реализация идёт tdd-инкрементами и по мере работы спецификации задачи и решения дополняются деталями или даже сущственно меняются, если в процессе реализации выяснилось, что первоначальный план оказался неудачным.
...
3. При необходимости дополните самостоятельно стартовые задачи в todo.md.
...
>- `040-*` — рабочие артефакты выполнения задачи.
>
...
`work-state.md` перезаписывается как актуальная контрольная точка для продолжения задачи в другой сессии или на другой машине и автоматически входит в контекст активной задачи, если содержит полезное состояние помимо структуры шаблона.
`todo.md` при этом остаётся списком запланированных и завершённых работ, а не журналом выполнения или состоянием передачи работы.
...
Фреймворк разделяет глобальную оркестрацию, инженерные правила, исполняемые процессы и память конкретной задачи.

```mermaid
flowchart TD
    SessionStart[Codex SessionStart] --> Baseline[Базовый контекст<br/>project-baseline.md]
    SessionStart --> Context[Корневой роутинг контекста<br/>context/index.md]
    SessionStart --> Config[Опциональная конфигурация фреймворка]
    Context --> ContextIndexes[Тематические индексы<br/>context/*.md]
    ContextIndexes --> RoutedContext[Соглашения и справочные материалы<br/>conventions/, references/, patterns/, artifacts/]
    Baseline --> TaskLoader[Загрузчик контекста задачи<br/>task-workdir/load_task_context.py]
    TaskLoader --> TaskResolver[Резолвер задачи<br/>task-workdir/resolve_task.py]
    TaskLoader -. при наличии задачи .-> TaskModule[Модуль task-workdir<br/>task-workdir/context.md]
    TaskModule --> Skill[Применимый скилл<br/>skills/**/SKILL.md]
    Caller[Вызывающая сторона] --> Skill
...
В первом ходе baseline вызывает загрузчик контекста задачи и использует его вывод напрямую.
Резолвер учитывает только идентификаторы активных задач, явно указанные отдельной строкой или сразу после ссылки на скилл, а при нескольких кандидатах изолированно выбирает единственное ясное соответствие по брифам.
Загрузчик вызывает резолвер и собирает полный контекст из правил task-workdir, брифа выбранной задачи и существующего содержательного `work-state.md` либо сообщает об отсутствии активной задачи.
После разрешения задачи модуль передаёт применимой операции конкретные привязки артефактов как явные смысловые входы и разрешённые вызывающей стороной пути результатов.

Hook `SessionStart` детерминированно загружает обязательные `project-baseline.md` и `context/index.md` в этом порядке, а затем опциональную конфигурацию фреймворка.
Корневой индекс контекста классифицирует фактическую запрошенную и запланированную работу и загружает все подходящие тематические индексы.

Baseline задаёт глобальную оркестрацию и ограничения поведения, а generic-скиллы содержат только собственные границы операции и загружают только внутренне необходимые их контракту зависимости.
Фреймворк не выбирает и не переключает роли, профили, режимы или персоны и не вводит другой классификатор запросов между фактической работой, маршрутизацией контекста и операциями.

| Слой | Назначение |
| --- | --- |
| `bootstrap/` | Устанавливает фреймворк в целевой репозиторий и подключает обязательный `SessionStart` hook. |
| `src/project-baseline.md` | Задаёт глобальную оркестрацию, ограничения поведения и загрузку контекста задачи. |
...
| `src/task-workdir/` | Содержит загрузчик контекста задачи, резолвер, условный контекст, task-workdir-скиллы и шаблоны стандартных артефактов задачи. |
...
Каждый файл в `src/conventions/` объявляет в YAML front matter хотя бы одно ключевое слово; рекомендуется использовать не более трёх.
...
| [`$fix-framework-context`](src/skills/fix-framework-context/SKILL.md) | Обновляет общий runtime-контекст Ergocode для исправления ошибок в поведении агента. |
