...
В этом варианте реализация задачи идёт по спецификации, но на первом этапе описываются только треобования и общие направление и ограничения решения, а затем они уточняются, дополняются и, при необходимости, изменяются по мере реализации.
...
После сбора первичной спецификации итеративно реализуйте инкременты до завершения реализации задачи.
...
1. Ручном - самостоятельно выбираете что делать дальше и обновляете рабоиче файлы задачи;
2. Полуавтоматическом - используйте скилл [`$advance-task`](src/skills/advance-task/SKILL.md) для автоматического выбора следующего шага, его исполнения и обновления рабочих файлов задачи;
3. Автоматическом - используйте скилл [`$implement-task-tdd`](src/skills/implement-task-tdd/SKILL.md), чтобы агент сам полностью реализовал задачу до конца, по умолчанию останавливаясь на ревью в ключевых точках.
