...
1. Accept the plan only when its request or task artifact identifies the selected case, specifies a production fix sufficiently to implement, and contains no unresolved implementation choice.
...
5. Keep investigation and the selected fix inside that boundary, except for compile-only call-site propagation forced by the selected change.
6. If the next necessary step would inspect or change a sibling endpoint, operation, mode, or scenario to justify the fix, stop and report the boundary instead of widening the plan.
7. If selecting the fix depends on an ambiguous requirement or implementation choice, report the unresolved question instead of guessing.
8. If the test contradicts task design, requires test edits, or cannot be fixed within production code, stop and report the blocker.
