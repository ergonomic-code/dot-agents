...
description: Find existing test cases for modules present in a ready structural diagram, usually `structure-chart/v1` YAML. Use when the input is a structural diagram and you need a grouped list of test cases by target module, with evidence from `*Test`, `*Api`, and `*HttpApi` classes under `src/test` and `src/testFixtures`, especially by scanning direct calls inside `// When` blocks.
---
...
2. Treat each collected module as a candidate target module.
3. For each candidate target module, grep `src/test` and `src/testFixtures` for direct symbol usage, receiver usage, and obvious test-side wrappers around that module.
...
7. Merge results by target module and write or return concrete test cases plus reportable uncovered module gaps.
...
Also include a test case when the direct `// When` call is one thin local wrapper away from the target module and the wrapper is resolved in the same test class or fixture.
Treat a wrapper as thin only when it forwards to one concrete target-module call without branching or alternative targets.
If the same test method touches several target modules from the diagram, include it under each matching module only when that module touch belongs to the `// When` block.

## Search Strategy

Prefer targeted code search from each target module over a full sweep of all tests.
Use names from the diagram as the first filter, then inspect only matched files and methods.
Look for:
- method names from the diagram;
- receiver names or injected fields bound to the target module;
- thin test-side wrappers that forward to the target module.
...
If the test reaches the controller through `*HttpApi` or `*Api`, attribute the case to the controller module from the diagram.
...
Stay within target modules present in the input diagram.
...
If no `// When` marker exists in a candidate test, infer the action phase from the test method semantics and use only target-module touches that belong to that action phase.
...
Direct coverage means at least one reported case is attributed to the target module.
Transitive coverage means the target module is reachable through diagram `calls` from a directly covered upstream module in the same execution path.
Treat a target module as complex when it owns branching, looping, validation, orchestration, side effects, IO, persistence, publication, or multiple meaningful project calls.
Treat a target module as simple when it only forwards, maps, wraps, extracts, constructs data, or delegates without its own branching or side effect.
For uncovered output, keep only:
- complex target modules without direct coverage;
- simple target modules without transitive coverage.

Transitive coverage does not suppress a complex target-module gap.
Do not list simple target modules transitively exercised by reported upstream cases.

## Output

If an output path is resolved, write concise Markdown grouped by target module to that file.
Otherwise return concise Markdown grouped by target module.
For each covered target module, list:
- test case name;
...
For reportable uncovered target modules, say no cases were found and state whether the gap is `complex without direct coverage` or `simple without transitive coverage`.
