...
Prefer `added` for new obligations.
Keep old cases unchanged unless they contradict new checks, would stop compiling, would fail under new conditions, or are baseline save/return cases for model field-set changes.
...
- A related old case plus a new behavior class is `unchanged` + `added`, not `changed`.
...
- if the output is AsciiDoc, render them as a two-column `|===` table with header cells `| Было | Стало`;
  ...
  - the left content cell is `a|`, then one `[source,text]` + `....` block with only the old case body, then the source reference if available;
  - the right content cell is `a|`, then one `[source,text]` + `....` block with only the updated full case body, then `Причина изменения: ...`, then `Изменения:` with a flat list;
