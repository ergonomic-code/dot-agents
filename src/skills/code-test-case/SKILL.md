...
If the user did not explicitly select several checks, all checks, or a named multi-case set, code only the explicitly selected `Check`.
...
Otherwise code the first not-yet-implemented `Check` in source order, or the first `Check` if implementation status is unknown.
