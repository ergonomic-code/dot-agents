...
Use when the selected example is an HTTP success path and the feature changes endpoint, response shape, or resource representation.

- Before adding the case or marking it green, use the active-stage `030-api-new.adoc` or `030-api-new-ir.json` as the target endpoint and response shape when present; otherwise use the controller contract.
