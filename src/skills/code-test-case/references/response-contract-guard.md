...
Use when the selected example is an HTTP success path and the requested change affects an endpoint, response shape, or resource representation.

- Before adding the case, use the caller-supplied API contract as the target endpoint and response shape when present; otherwise use the controller contract.
