...
- Before role selection or routed-context loading, pass the initial user request through stdin to `framework_checkout_root/src/task-workdir/load_task_context.py --repo-root "$(git rev-parse --show-toplevel)"` exactly once and treat its stdout as authoritative session task context.
